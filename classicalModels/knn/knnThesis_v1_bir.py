...
resultGraphBase="42multiExpResulBir\\"
# load the new file
...
for lagLat in lagLatRange:
    for ng in neighbourN:
        print("Current Lopps param expNum:",expNum," lagLat:",lagLat," neighbourN:",ng)
...
#        build_model_M(n_timesteps, n_features,out_vars,nNurons,auxInputDim=0,mergemod='concat')
        ...
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"trainValLossLine"])
        ...
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"testMSELocHeatmap"])
        
        test_res_sum2D=conv1To2DPred(test_res_sum[0],sizeMat,pad)
        ax = sns.heatmap(test_res_sum2D)
        plt.savefig(resultGraphBase+key+'.png')
        plt.clf()
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTrain"])
        ...
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTest"])
...
#        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
#        resultDicmain[key]=mseTestScaled
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"test_y"])
        resultDicmain[key]=test_y
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"test_yhat"])
