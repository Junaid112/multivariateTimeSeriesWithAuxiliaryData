...
resultGraphBase="42multiExpResulBirAux\\"
...
for auxIdx in auxDataIdx:
    ...
    #        for expNum in expNumRange:
            expNum=1
            print("Current Lopps param auxIdx:",auxIdx," lagLat:",lagLat," expNum:",expNum)
...
#            model.fit(train_x, train_y)
    ...
    #        print(dataSrc+'1_Train MSE: %.3f' % modelTrainHist.history['loss'][-1])
            ...
            test_res=test_res**2
            test_res=np.reshape(test_res,(test_res.shape[0],lagLat,test_res.shape[1]//lagLat))
            test_res_sum=np.sum(test_res,axis=0)
            
            key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"testMSELocHeatmap"])
    ...
    #            resultDicmain[key]=modelTrainHist.history['val_loss']
            key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTest"])
            resultDicmain[key]=mseTest
            key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
...
#            key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
#            resultDicmain[key]=mseTestScaled
            key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"test_y"])
            resultDicmain[key]=test_y
            key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"test_yhat"])
