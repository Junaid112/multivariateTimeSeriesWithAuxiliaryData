...
resultGraphBase="42multiExpResul2BirAux\\"
...
for auxIdx in auxDataIdx:
    for lag in lagLatRange:
        ...
        #            n_timesteps, n_features= train_x.shape[1], train_x.shape[2]
                model = KNeighborsRegressor(n_neighbors=ng).fit(train_x, train_y)
    #            model.fit(train_x, train_y)
        ...
        #        print(dataSrc+'1_Train MSE: %.3f' % modelTrainHist.history['loss'][-1])
                ...
                key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lag),str(Lat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"testMSELocHeatmap"])
        ...
        #            resultDicmain[key]=modelTrainHist.history['val_loss']
                key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lag),str(Lat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTest"])
                resultDicmain[key]=mseTest
                key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lag),str(Lat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
                resultDicmain[key]=mseTestScaled
    #            key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
    #            resultDicmain[key]=mseTestScaled
    #            key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
    #            resultDicmain[key]=mseTestScaled
                key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lag),str(Lat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"test_y"])
                resultDicmain[key]=test_y
                key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lag),str(Lat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"test_yhat"])
