...
resultGraphBase="xgBoost\\41multiExpResulBirAux\\"
...
#        print(dataSrc+'1_Train MSE: %.3f' % modelTrainHist.history['loss'][-1])
        ...
        key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"testMSELocHeatmap"])
...
#            resultDicmain[key]=modelTrainHist.history['val_loss']
        key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTest"])
        resultDicmain[key]=mseTest
        key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
...
#        key="_".join([dataSrc,auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
#        resultDicmain[key]=mseTestScaled
        key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"test_y"])
        resultDicmain[key]=test_y
        key="_".join([dataSrc,auxDataName[auxIdx],auxDataName[auxIdx],modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"test_yhat"])
