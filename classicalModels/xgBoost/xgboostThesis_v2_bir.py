...
resultGraphBase="gBoost\\41multiExpResul2Bir\\"
# load the new file
...
for lag in lagLatRange:
    ...
    #        build_model_M(n_timesteps, n_features,out_vars,nNurons,auxInputDim=0,mergemod='concat')
            ...
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"trainValLossLine"])
            ...
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"testMSELocHeatmap"])
            
            test_res_sum2D=conv1To2DPred(test_res_sum[0],sizeMat,pad)
            ax = sns.heatmap(test_res_sum2D)
            plt.savefig(resultGraphBase+key+'.png')
            plt.clf()
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"mseTrain"])
            ...
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"mseTest"])
            resultDicmain[key]=mseTest
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
    ...
    #        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),str(ng),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
    #        resultDicmain[key]=mseTestScaled
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"test_y"])
            resultDicmain[key]=test_y
            key="_".join([dataSrc,modelName,str(expNum),str(lag),str(Lat),"_".join(np.array(nNurons[0]).astype(str)),"test_yhat"])
