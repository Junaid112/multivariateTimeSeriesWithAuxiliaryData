...
#    auxlayer1=100
    ...
    rep1=RepeatVector(n_outputs)(lstm1)
...
# load the new file
dataset = read_csv(dataFileBir, header=0, infer_datetime_format=True, parse_dates=['date'], index_col=0).fillna(0)
...
for expNum in expNumRange:
    for lagLat in lagLatRange:
        ...
        #Key #dataSrc_model_expNum_in_out_nNurons_variableName
        # design network
        n_timesteps, n_features= train_x.shape[1], train_x.shape[2]
        model = build_model_M(n_timesteps, n_features,n_features,nNurons[0])
        # fit network
        modelTrainHist = model.fit(train_x, train_y, epochs=epochs, batch_size=batch_size, validation_data=(test_x, test_y), verbose=verbose, shuffle=False)
        # plot history
        ...
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"trainValLossLine"])
        ...
        plt.clf()
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"trainLoss"])
        ...
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTest"])
        resultDicmain[key]=mseTest
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
        resultDicmain[key]=mseTestScaled
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
        resultDicmain[key]=mseTestScaled
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"mseTestScaled"])
        resultDicmain[key]=mseTestScaled
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"test_y"])
        resultDicmain[key]=test_y
        key="_".join([dataSrc,modelName,str(expNum),str(lagLat),str(lagLat),"_".join(np.array(nNurons[0]).astype(str)),"test_yhat"])
